...
🧹 Desinstalação / Rollback (Linux):Se você optou pelo Modo Virtual (venv), a desinstalação consiste simplesmente em apagar a pasta local.Remoção Manual (Apenas o ambiente virtual):deactivate 2>/dev/null
rm -rf ~/venv-devnet
...
🧹 Desinstalação / Rollback (macOS):Se você utilizou o Modo Virtual (venv), a desinstalação pode ser feita limpando a pasta do ambiente.Remoção Manual (Apenas o ambiente virtual):deactivate 2>/dev/null
rm -rf ~/venv-devnet
